...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    **Diff file:** [DIFF_FILE]
    ...
    ## Part 2: Code Quality & NihongoCards Standards

    **NihongoCards Mandatory Checks:**
    - **NO Lombok**: Verify that no `@Getter`, `@Setter`, `@Data`, `@NoArgsConstructor`, or Lombok imports exist (automatic rejection).
    - **SM-2 is_learned**: Verify that words marked learned never lose `is_learned = true` when evaluated with score < 3.
    - **Layering & DTOs**: Controller validates with `@Valid` and returns DTO; Service has `@Transactional` for state-changing operations; no raw Entity exposed.
    - **Styling**: Vanilla CSS (no Tailwind classes).
    - **Flyway**: DB changes include migration file `V<n>__...sql` compatible with H2 and MySQL.
