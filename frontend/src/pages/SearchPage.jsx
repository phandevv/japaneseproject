  ...
  return (
    <div className="container animate-fade-in" style={{ padding: '40px 20px' }}>
      <div style={{ textAlign: 'center', marginBottom: '40px' }}>
        <h1 style={{ fontSize: '2.5rem', marginBottom: '15px' }}>Dictionary Search</h1>
        <p style={{ color: 'var(--text-secondary)' }}>Search by Kanji, Hiragana, meaning, or Romaji (Hán Việt)</p>
      </div>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <div className="flex-between" style={{ marginBottom: '20px' }}>
            <h3 style={{ color: 'var(--text-secondary)' }}>
              Found {results.totalElements} results
            </h3>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            {results.content.length === 0 && (
              <div className="card flex-center" style={{ padding: '40px', color: 'var(--text-secondary)' }}>
                No vocabulary found matching "{query}"
              </div>
            )}
          </div>
          ...
          {results.totalPages > 1 && (
            <div className="flex-center" style={{ gap: '20px', marginTop: '30px' }}>
              ...
              <span>Page {results.number + 1} of {results.totalPages}</span>
              <button 
                className="btn-icon" 
